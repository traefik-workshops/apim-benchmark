...
module "grafana-stack" {
  ...
  # Enable snapshots and the login form:
  # - `[snapshots]` keys un-hide the "Publish to snapshot.raintank.io"
  #   entry in the Share > Snapshot dropdown (disabled by default in
  #   recent grafana/grafana chart versions).
  # - `auth.disable_login_form = false` un-hides the login form so an
  #   admin can sign in. Anonymous access is still configured in the
  #   grafana-stack module (auth.anonymous.enabled=true, org_role=Admin),
  #   so casual viewing continues to work without login -- but snapshot
  #   creation is hardcoded-blocked for anonymous users since Grafana
  #   7.4.2 (grafana/grafana#32585, closed as not planned). To actually
  #   publish a snapshot from the UI you must log in; the login form has
  #   to be reachable for that.
  # - `adminPassword` seeds the chart-managed `grafana` secret on first
  #   install. OK to hard-code in this repo: the cluster is an ephemeral
  #   benchmark rig (same rationale as Keycloak's admin/admin), not a
  #   production system hosting real data.
  #
  # NOTE: Grafana only reads adminPassword on *fresh* install. If Grafana
  # is already running with a different password, change it in-place:
  #   kubectl -n dependencies exec deploy/grafana -- \
  #     grafana cli admin reset-admin-password topsecretpassword
